...
`CrashLoopBackOff`는 에러가 발생했다는 뜻이 아니라 너무 짧은 시간 안에 컨테이너가 계속 종료되고 있다는 뜻이다. 그래서 kubelet이 스스로 이렇게 판단한다 — 이건 문제가 있을 수도 있으니 잠깐 쉬었다가 다시 실행하자. 이것이 CrashLoopBackOff이다.
