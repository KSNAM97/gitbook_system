...
1. SNS 콘솔의 좌측 메뉴에서 [주제]를 선택하고 [주제 생성]을 클릭한다.
2. 유형은 [표준]을 선택한다(순서 보장이 필요 없는 일반 알림이므로 FIFO가 아닌 표준 주제를 사용한다).
3. 이름을 입력한다(예: `s3-upload-sms-email`).
4. 나머지 설정은 기본값으로 두고 [주제 생성]을 선택한다.

### 2) 구독(Subscription) 생성 — 이메일로 알림 받기

1. 방금 만든 주제 상세 화면에서 [구독 생성]을 선택한다.
2. 프로토콜은 [이메일]을 선택하고, 엔드포인트에 알림을 받을 이메일 주소를 입력한 뒤 [구독 생성]을 클릭한다.
3. 입력한 이메일로 "AWS Notification - Subscription Confirmation" 메일이 도착한다. 메일 본문의 **Confirm subscription** 링크를 클릭해야 구독이 실제로 활성화된다(클릭 전까지는 Pending confirmation 상태로 메시지를 받지 못한다).

### 3) IAM 정책 — Lambda에 SNS 발행 권한 부여
...
### 4) Lambda 함수 생성과 S3 트리거 연결

1. Lambda 콘솔에서 [함수 생성]을 선택한다.
2. [새로 작성]을 선택하고, 함수 이름을 입력한다(예: `s3-upload-notifier`). 런타임은 [Python 3.x]를 선택한다.
3. [함수 생성]을 클릭해 빈 함수를 만든다.
4. 함수 상세 화면의 [코드] 탭에서 기본 코드를 지우고 아래 6)의 Python 코드를 붙여넣은 뒤 **Deploy**를 클릭한다.
5. [구성] → [트리거 추가]를 선택하고, 소스로 [S3]를 선택한다.
6. 버킷을 지정하고, 이벤트 유형은 [모든 객체 생성 이벤트](`PUT`, `POST`, `COPY`, 멀티파트 업로드 완료를 모두 포함)를 선택한다. 접두사(Prefix)에 `uploads/`를 입력하면 S3 트리거 단계에서부터 해당 경로만 필터링할 수 있다(단, 이 실습에서는 코드 레벨 필터도 함께 확인하기 위해 접두사를 비워두고 진행해도 된다).
7. [추가]를 클릭하면 S3 버킷의 [속성] → [이벤트 알림]에도 이 Lambda 트리거가 자동으로 등록된 것을 확인할 수 있다.

### 5) Lambda 실행 역할에 SNS 발행 권한 연결

Lambda 함수를 만들면 기본 실행 역할이 자동으로 생성되지만, 이 역할에는 SNS 권한이 없다. 3)에서 만든 정책을 이 역할에 붙여야 한다.

1. Lambda 함수의 [구성] → [권한] 탭에서 **역할 이름**을 클릭하면 IAM 콘솔로 이동한다.
2. IAM 역할 화면에서 [권한 추가] → [인라인 정책 생성]을 선택한다.
3. [JSON] 탭으로 전환해 3)의 정책 JSON을 붙여넣는다(Topic ARN은 실제 생성한 주제의 ARN으로 교체한다 — 주제 상세 화면 상단에서 ARN을 복사할 수 있다).
4. 정책 이름을 입력하고(예: `sns-publish-s3-upload`) [정책 생성]을 클릭한다.

### 6) Python 코드 — S3 이벤트를 받아 SNS로 발행
...
### 1) S3 버킷에서 EventBridge 알림 활성화

S3의 기본 이벤트 알림(실습 1에서 쓴 방식)과 EventBridge는 별개의 경로다. S3 이벤트를 EventBridge로 보내려면 버킷에서 이 기능을 먼저 켜야 한다.

1. 대상 S3 버킷의 [속성] 탭으로 이동한다.
2. 맨 아래 [Amazon EventBridge] 항목에서 [편집]을 선택하고 [켜기]를 체크한 뒤 저장한다.

### 2) EventBridge 규칙 생성

1. Amazon EventBridge 콘솔의 좌측 메뉴에서 [규칙]을 선택하고 [규칙 생성]을 클릭한다.
2. 규칙 이름을 입력한다(예: `s3-upload-rule`). 이벤트 버스는 기본값인 `default`를 사용한다.
3. 규칙 유형은 [이벤트 패턴이 있는 규칙]을 선택한다.
4. 이벤트 패턴 단계에서 이벤트 소스는 [AWS 서비스], 서비스는 [Simple Storage Service (S3)], 이벤트 유형은 [Amazon S3 Event Notification]을 선택한다. 특정 버킷·접두사까지 좁히려면 화면 하단의 [사용자 지정 패턴(JSON 편집기)]으로 전환해 아래 3)의 JSON을 직접 붙여넣는다.
5. 대상(Target) 단계에서 [SQS 큐]를 선택하고, 이벤트를 전달받을 큐를 지정한다.
6. [다음] → [규칙 생성]으로 마무리한다.
