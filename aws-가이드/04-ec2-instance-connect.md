...
![인스턴스에 연결 - EC2 인스턴스 연결 탭에서 기본값으로 연결 버튼 클릭](../aws/assets/ec2-connect-modal.png)

EC2 인스턴스에 바로 접속된다. 리눅스 명령어를 사용하여 내부 폴더를 살펴보거나 새로운 폴더를 생성할 수 있다. 외부 패키지를 설치하면 깃에 올려둔 파일을 다운로드 받아 EC2 내에서 설치할 수도 있다.

![EC2 Instance Connect로 브라우저에서 바로 접속된 콘솔 화면](../aws/assets/ec2-instance-connect-browser-console.png)
...
![프라이빗 키 파일 권한이 너무 열려있어 접속이 거부되는 경고 (UNPROTECTED PRIVATE KEY FILE)](../aws/assets/ssh-key-permission-denied.png)
...
![chmod 700 적용 후 SSH로 정상 접속된 터미널 화면](../aws/assets/ssh-connect-success-terminal.png)
