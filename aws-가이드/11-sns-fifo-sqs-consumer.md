...
1. SQS 콘솔에서 [대기열 생성]을 선택한다.
2. 유형은 [FIFO]를 선택한다.
3. 이름을 입력한다. **FIFO 큐는 이름 끝에 반드시 `.fifo`를 붙여야 한다**(예: `my-test-queue.fifo`).
4. 콘텐츠 기반 중복 제거를 쓰지 않을 경우(프로듀서가 Deduplication ID를 직접 지정하는 경우) 비활성 상태로 두고, 간단한 테스트만 하려면 [콘텐츠 기반 중복 제거 사용]을 체크해도 된다.
5. Visibility Timeout·메시지 보존 기간 등 나머지 값은 기본값으로 두고 [대기열 생성]을 클릭한다.

### 2) SNS FIFO 주제 생성

1. SNS 콘솔에서 [주제 생성]을 선택한다.
2. 유형은 [FIFO]를 선택한다.
3. 이름을 입력한다. 이 역시 끝에 `.fifo`가 필요하다(예: `my-test-topic.fifo`).
4. [콘텐츠 기반 메시지 중복 제거] 사용 여부를 선택한다(Deduplication ID를 코드에서 직접 넘기지 않는다면 체크한다).
5. [주제 생성]을 클릭한다.

### 3) SQS FIFO 큐를 SNS FIFO 주제의 구독자로 연결

1. 방금 만든 SNS FIFO 주제 상세 화면에서 [구독 생성]을 선택한다.
2. 프로토콜은 [Amazon SQS]를 선택하고, 엔드포인트에서 1)에서 만든 SQS FIFO 큐를 선택한다(같은 계정의 큐라면 목록에서 바로 선택할 수 있다).
3. [구독 생성]을 클릭한다. 콘솔에서 이 구독을 생성하면 SQS 큐의 액세스 정책에 "이 SNS 주제가 메시지를 보낼 수 있다"는 권한이 자동으로 추가된다(수동으로 만들 경우에는 큐의 [액세스 정책]에 해당 SNS 주제 ARN을 `Principal`로 허용하는 정책을 직접 추가해야 한다).

### 4) EC2 인스턴스에 SQS 접근 권한 부여

user-data로 띄울 Python 소비자가 `sqs:ReceiveMessage`/`sqs:DeleteMessage`/`sqs:GetQueueUrl` 권한을 쓸 수 있도록, EC2에 연결할 IAM 역할을 먼저 준비한다.

1. IAM 콘솔에서 [역할] → [역할 생성]을 선택하고, 신뢰할 수 있는 엔터티 유형은 [AWS 서비스], 사용 사례는 [EC2]를 선택한다.
2. 권한 정책에서 `AmazonSQSFullAccess`(테스트용) 또는 큐 ARN 하나로 범위를 좁힌 커스텀 정책을 연결한다.
3. 역할 이름을 입력하고(예: `ec2-sqs-consumer-role`) 생성한다.

### 5) EC2 인스턴스 시작과 user-data 등록

1. EC2 콘솔에서 [인스턴스 시작]을 선택한다.
2. AMI(Amazon Linux 2023 등)와 인스턴스 유형(`t3.micro` 등)을 지정한다.
3. [고급 세부 정보]를 펼치고 **IAM 인스턴스 프로파일**에 4)에서 만든 역할을 지정한다.
4. 같은 [고급 세부 정보] 안의 **사용자 데이터** 입력란에 아래 6)의 user-data 스크립트를 붙여넣는다.
5. [인스턴스 시작]을 클릭한다. 부팅이 완료되면 user-data 스크립트가 자동 실행되어 Python·boto3 설치, 소비자 스크립트 생성, 백그라운드 실행까지 한 번에 처리된다.
...
1. SNS 콘솔에서 2)에서 만든 FIFO 주제를 선택하고 [메시지 게시]를 클릭한다.
2. **메시지 그룹 ID**(FIFO 주제 전용 필드)에 임의의 그룹 이름을 입력한다(예: `my-test-group`). 같은 그룹 ID로 보낸 메시지끼리만 순서가 보장된다는 점을 이 실습에서 실제로 확인한다.
3. 콘텐츠 기반 중복 제거를 껐다면 **메시지 중복 제거 ID** 필드도 채워야 한다(매번 다른 값을 넣는다).
4. 제목(Subject)과 메시지 본문에 아래 JSON을 입력하고 [메시지 게시]를 클릭한다.
