...
### Step 5. 서브커맨드형 스크립트 골격 (위치 매개변수 + case)
...
> **참고:** 원본 예제처럼 `$1`, `$2` 만 검증하면 `10 20 A` 가 통과되어 합계가 `30` 으로 잘못 계산된다. 가변 인자는 전체를 순회하며 검증해야 한다.
...
> **참고:** `systemctl status` 는 서비스가 정지 상태일 때 종료 코드 `3` 을 반환한다. 따라서 status 결과를 `$?` 로 성공/실패 판정에 쓰면 안 되고, 활성 여부는 `systemctl is-active` 로 확인한다.
...
> **참고:** 저장 디렉터리(`$2`)를 백업 대상(`$1`) 하위에 지정하면 자기 자신을 다시 압축하려는 재귀가 발생한다. 대상과 저장 경로는 반드시 서로 독립된 위치로 지정한다.
