...
> RAID와 LVM은 목적이 다르므로 함께 쓰기도 합니다. 결함 허용이 필요하면 RAID(또는 LVM RAID)로 보호 계층을 만들고, 그 위에서 LVM으로 용량을 유연하게 관리하는 조합이 일반적입니다.
...
> ⚠️ LVM 계층은 위에서 아래(LV→VG→PV) 순으로 삭제해야 안전합니다. 하위 계층부터 지우면 참조 오류가 발생할 수 있습니다.
