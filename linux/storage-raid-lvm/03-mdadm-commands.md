...
> **주의:**  **파괴적 명령 주의:** `wipefs -a`와 `--zero-superblock`은 데이터 접근을 파괴할 수 있습니다. 실행 전 반드시 `lsblk`, `mdadm --detail`로 대상 디스크가 맞는지 확인합니다. `--stop`은 현재 조립된 RAID의 **동작만 중지**하며 구성 자체를 삭제하지 않습니다.
...
> - 파괴적 명령 전 대상 디스크 재확인
> - 관련:  RAID 개념 & Hardware vs Software RAID ·  RAID 레벨별 특징 (Linear·0·1·5·6) ·  종합실습 RAID 5 + Spare 구성 & 장애 복구
