...
> 파티션을 생략하고 디스크 전체를 사용하거나 LVM·RAID·암호화를 추가하는 구성도 가능하다. 위 순서는 일반적인 파티션 기반 교육·운영 템플릿이다.
...
> 아래의 `/dev/sdb`는 예시다. 실제 장치의 모델·일련번호·크기·사용 상태를 확인한 뒤 대체한다.
...
> 마운트 옵션은 애플리케이션 요구사항을 검토한 뒤 적용한다.
