...
> `mkdir -p`는 중첩 경로를 편리하게 생성하지만 파일시스템 전체 작업의 원자성을 보장하는 명령은 아니다.
...
> `rm`으로 삭제한 항목은 일반적으로 휴지통으로 이동하지 않는다. 백업이 없다면 복구가 매우 어렵거나 불가능할 수 있다.
...
> RHEL 계열의 대화형 Root 환경에서는 `rm`, `cp`, `mv`에 `-i` alias가 설정되어 있을 수 있다. 확인 메시지는 `rm` 자체의 기본 동작이 아닐 수 있다.
...
> `${DIR:?}`와 `set -u`는 빈 변수와 미정의 변수를 방어한다. 하지만 `/home/guest/data` 대신 `/home/guest`처럼 잘못된 값이 들어간 경우까지 자동으로 막지는 못하므로 허용 경로 검사도 필요하다.
...
> `rm -r /home/guest/app/*`는 `app` 디렉터리 자체를 삭제하지 않으며 숨김파일을 누락할 수 있다.
