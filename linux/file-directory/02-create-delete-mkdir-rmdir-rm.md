...
> `rm`은 일반적으로 휴지통을 사용하지 않는다. 파일시스템과 백업 상태에 따라 복구가 매우 어렵거나 불가능할 수 있다.
...
> RHEL 계열의 대화형 Root 환경에서는 `rm`, `cp`, `mv`에 `-i` alias가 설정되어 있을 수 있다. 명령 자체의 기본 동작이라고 단정해서는 안 된다.
...
> `${DIR:?}`와 `set -u`는 미정의·빈 변수 문제를 줄이지만, 잘못된 비어 있지 않은 경로까지 막아주지는 않는다. 허용 경로 검증이 함께 필요하다.
