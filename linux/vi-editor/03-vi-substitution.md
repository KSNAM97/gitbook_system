...
> **참고:** Vim의 `\<`, `\>`는 `'iskeyword'` 설정을 기준으로 단어 경계를 판단한다.
...
> **주의:** `:%s/^\s*$//`는 줄을 삭제하지 않고 줄 안의 공백만 제거한다.
...
> **참고:** `n` 플래그가 있으면 실제 치환은 수행하지 않고 일치 개수만 보고한다.
...
> **참고:** 위 명령은 현재 버퍼 내용을 `diff`의 표준 입력으로 전달하므로 아직 원본 파일에 저장하지 않은 변경도 비교할 수 있다.
